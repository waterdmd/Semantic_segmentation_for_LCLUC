...
# ##============================Fusion median Data=============================
train_ds = SegmentationDatasetFusion(data_path='/data/data/Fusion/Train', transform=None)
# sampler = torch.utils.data.WeightedRandomSampler(train_ds.weights, len(train_ds.weights))
train_dataloader = DataLoader(train_ds, batch_size=BS, num_workers = 16, pin_memory=True)
val_ds = SegmentationDatasetFusion(data_path='/data/data/Fusion/Val', transform=None)
# sampler1 = torch.utils.data.WeightedRandomSampler(val_ds.weights, len(val_ds.weights))
val_dataloader = DataLoader(val_ds, batch_size=BS,num_workers = 16, pin_memory=True)
...
input_channels = 21  # Update this to 8, 16, or 24 based on your dataset
...
# %%
# wights = glob.glob("HRNET*.pth")
# if wights:
#     model.load_state_dict(torch.load(f'HRNET_epochs_20_crossentropy_state_dict.pth'))
...
# Log or print validation metrics and loss
    ...
    if np.median(running_val_loss) < min_loss:
        ...
        torch.save(model.state_dict(), f'HRNET_epochs_{EPOCHS}_crossentropy_state_dict.pth')
    ...
    print(f"Epoch: {e}: Medin Train Loss: {np.mean(running_train_loss)}, Val Loss: {np.mean(running_val_loss)}")
