...
# %%
wights = glob.glob("UNET*.pth")
if wights:
    model.load_state_dict(torch.load(f'UNET_epochs_{EPOCHS}_crossentropy_state_dict.pth'))
...
# Log or print validation metrics and loss
    ...
    if np.median(running_val_loss) < min_loss:
        ...
        torch.save(model.state_dict(), f'UNET_epochs_{EPOCHS}_crossentropy_state_dict.pth')
